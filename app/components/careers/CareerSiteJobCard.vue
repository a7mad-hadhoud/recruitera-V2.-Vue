...
<template>
  ...
  >
    <div class="mb-2 flex flex-wrap gap-1.5">
      <span class="rounded-full px-2.5 py-0.5 text-[10.5px] font-bold" :style="{ background: `${site.primaryColor}18`, color: site.primaryColor }">{{ props.job.employmentType || 'Full-time' }}</span>
      <span class="rounded-full px-2.5 py-0.5 text-[10.5px] font-bold" :style="{ background: `${site.headerColor}14`, color: site.headerColor }">{{ props.job.collar === 'white' ? t('filter_job_type_white') : t('filter_job_type_blue') }}</span>
    </div>
