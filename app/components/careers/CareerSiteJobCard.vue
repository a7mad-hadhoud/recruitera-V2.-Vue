...
<template>
  ...
  >
    <div class="mb-2 flex flex-wrap gap-1.5">
      <span class="rounded-md px-2 py-0.5 text-[10.5px] font-bold" :style="{ background: `${site.primaryColor}18`, color: site.primaryColor }">{{ props.job.employmentType || 'Full-time' }}</span>
      <span class="rounded-md bg-[var(--brand-preview-surface-alt)] px-2 py-0.5 text-[10.5px] font-bold text-[var(--brand-preview-text-label)]">{{ props.job.collar === 'white' ? t('filter_job_type_white') : t('filter_job_type_blue') }}</span>
    </div>
    <div class="mb-1 text-[14.5px] font-semibold text-[var(--brand-preview-text-heading)]">{{ props.job.title }}</div>
    <div class="mb-2.5 line-clamp-2 text-[12.5px] leading-[1.5] text-[var(--brand-preview-text-secondary)]">{{ props.job.description }}</div>
