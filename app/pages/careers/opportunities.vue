...
// Dropdown/pill filters — apply instantly on selection (PRD Feature 5 AC).
const location = ref('')
const category = ref('')
      ...
      <div class="mx-auto max-w-[1200px] px-6 py-8">
        ...
        <div class="flex flex-col gap-6 sm:flex-row">
          ...
          <aside class="hidden w-[220px] shrink-0 flex-col gap-4 sm:flex">
            ...
            <div>
              <label class="mb-1.5 block text-[12px] font-semibold text-[var(--brand-preview-text-label)]">{{ t('filter_job_type') }}</label>
              <div class="flex flex-wrap gap-1.5">
                <button
                  ...
                  :class="jobType === opt.v ? 'text-white' : 'border border-[var(--brand-preview-border)] text-[var(--brand-preview-text-label)]'"
                  :style="jobType === opt.v ? { background: site.primaryColor } : {}"
                  @click="jobType = opt.v as typeof jobType"
                >{{ opt.l }}</button>
              </div>
            </div>
            <button
                ...
                >
                  <div class="min-w-0 flex-1">
                    <div class="mb-1 flex flex-wrap items-center gap-1.5">
                      ...
                      <span class="rounded-full px-2 py-0.5 text-[10.5px] font-bold" :style="{ background: `${site.primaryColor}18`, color: site.primaryColor }">{{ j.employmentType || 'Full-time' }}</span>
                      <span class="rounded-full px-2 py-0.5 text-[10.5px] font-bold" :style="{ background: `${site.headerColor}14`, color: site.headerColor }">{{ j.collar === 'white' ? t('filter_job_type_white') : t('filter_job_type_blue') }}</span>
                    </div>
